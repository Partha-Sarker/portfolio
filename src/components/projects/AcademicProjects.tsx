...
const AcademicProjects: React.FC<AcademicProjectsProps> = ({ projects }) => {
    ...
    const academicProjects = projects.filter(project => project.type === 'academic');
    ...
    const featuredAcademicProjects = academicProjects.filter(project =>
        project.achievements && project.achievements.length > 0
    );
    const regularAcademicProjects = academicProjects.filter(project =>
        !project.achievements || project.achievements.length === 0
    );
    ...
    return (
        <Box sx={{ mb: 6 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 4 }}>
                <SchoolIcon sx={{ mr: 2, color: theme.palette.secondary.main, fontSize: '2rem' }} />
                ...
                <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr', md: '1fr 1fr' }, gap: { xs: 2, md: 3 } }}>
                    <Box>
                        ...
                        <Typography variant="body2" color="text.secondary" sx={{ lineHeight: 1.6 }}>
                            • Game Development with Unity and C#<br />
                            • Web Development with Laravel, PHP, and MySQL<br />
                            • Mobile App Development with Android Studio and Java<br />
                            • Computer Graphics Programming with OpenGL and C++<br />
